...
double N;                       //the discretization number
...
complex<double> phi_hes (double r,double q,double omega,double lamda,double nu_bar,double u_0,double eta,double rho,double T) {
...
double U(double k,int C_P,double a,double b){
  ...
  else if(C_P==-1)    //for put
    return 2*(-chi(a,0,a,b,k)+psi(a,0,a,b,k))/(b-a);
};

//calculate the value of a
double a(double r,double q,double T,double lamda,double nu_bar,double u_0,double eta,double rho) {
  double c1,c2;
  ...
  return (c1-12*sqrt(abs(c2)));   
};

//calculate the value of b
double b(double r,double q,double T,double lamda,double nu_bar,double u_0,double eta,double rho) {
  double c1,c2;
  c1 = ((r-q)*T+(1-exp(-lamda*T))*((nu_bar-u_0)/(2*lamda))-0.5*nu_bar*T);
  c2 = ((1/(8*lamda*lamda*lamda))*((eta*T*lamda*exp(-lamda*T)*(u_0-nu_bar)*(8*lamda*rho-4*eta))+(lamda*rho*eta*(1-exp(-lamda*T))*(16*nu_bar-8*u_0))+(2*nu_bar*lamda*T*(-4*lamda*rho*eta+eta*eta+4*lamda*lamda))+(eta*eta*((nu_bar-2*u_0)*exp(-2*lamda*T)+nu_bar*(6*exp(-lamda*T)-7)+2*u_0))+(8*lamda*lamda*(u_0-nu_bar)*(1-exp(-lamda*T)))));
  return (c1+12*sqrt(abs(c2)));    
...
double COS(double T,double lamda,double nu_bar,double u_0,double eta,double rho,double r,double q,double S_0,double K,int C_P,int N) {
  int k;
  double ct1,ct2;
  double x;
  ct1 = a(r,q,T,lamda,nu_bar,u_0,eta,rho);         //ct1 is the lower bound 
  ct2 = b(r,q,T,lamda,nu_bar,u_0,eta,rho);         //ct2 is the upper bound
  x = log(S_0/K);
  complex<double> c_i(0,1);
  complex<double> sum (0,0);
  
  //given the log-asseat price, the option price can be found by IFFT
  sum = 0.5*real(phi_hes(r, q, 0,lamda,nu_bar,u_0,eta,rho,T)*U(0,C_P,ct1,ct2)*exp(0.0));
  for (k=1;k<N;k++) {
      complex<double> count (k,0);
      sum=sum+real(phi_hes(r, q, k*pi/(ct2-ct1),lamda,nu_bar,u_0,eta,rho,T)*U(k,C_P,ct1,ct2)*exp(c_i*count*pi*(x-ct1)/(ct2-ct1)));
  }
...
int main()
{
	double min[5]={0,0,0,0,-1};
	double max[5]={20,1,1,1,1};
	int n=5;
	int nPop=75;
	...
	for (i=0; i < nPop; i++)
	{
		...
		double t_s=0.0;
      	for (int k = 0; k<rf.size(); k++){
			t = COS(T[k], Element[i][0], Element[i][1], Element[i][2], Element[i][3], Element[i][4], rf[k], q[k], FT[k], K[k], opType[k], 64) - mktPrice[k];
			t=t*t*vega[k];
			t_s=t_s+t;
		}
		popEnergy[i] = t_s;
	...
	for (int generation=0;(generation <= maxGenerations) && !bAtSolution;generation++){
		cout  << generation+1<<"\t"<<"\t"<<trialSolution[0]<<" "<<trialSolution[1]<<" "<<trialSolution[2]<<" "<<trialSolution[3]<<" "<<trialSolution[4]<<" "<<endl;
		cout  <<"\t"<< bestEnergy << "\t"  << bestSolution[0] << " " << bestSolution[1] << " " << bestSolution[2] << " " << bestSolution[3] << " " << bestSolution[4] << endl<<endl;
		for (candidate=0; candidate < nPop; candidate++)
		{
			...
			double t_s=0.0;
      		for (int i = 0; i<rf.size(); i++){
				t = COS(T[i], trialSolution[0], trialSolution[1], trialSolution[2], trialSolution[3], trialSolution[4], rf[i], q[i], FT[i], K[i], opType[i], 64)-mktPrice[i];
				t=t*t*vega[i];
				t_s = t_s+t;
			...
			if (trialEnergy < popEnergy[candidate])
			{
				...
				// Check if all-time low & fulfill feller condition
				if (trialEnergy < bestEnergy&&((2*trialSolution[0]*trialSolution[1]-pow(trialSolution[3],2))>0)) //add "&&((2*trialSolution[0]*trialSolution[1]-pow(trialSolution[3],2))>0)" if we want to ensure feller condition holds true
				{
	...
	cout<<"\n\nBest Coefficients:\n"<<endl;
	 ...
	 printf("Lowest Cost: %.10lf\n", bestEnergy);
    
    if(2*bestSolution[0]*bestSolution[1] - pow(bestSolution[3],2)>0){
